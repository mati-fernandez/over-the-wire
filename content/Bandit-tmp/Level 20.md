...
2. **Client Terminal:** In a parallel session, I ran the binary: `./suconnect 12345` The binary successfully connected to the `nc` server, received the password, validated it internally (thanks to its SETUID permissions), and returned the [[Informática/Ciberseguridad/OverTheWire/content/Bandit-tmp/Level 21]] credential.
